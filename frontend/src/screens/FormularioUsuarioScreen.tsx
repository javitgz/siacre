...
import { actualizarUsuario, crearUsuario, obtenerRoles, Rol, Usuario } from '../utils/api';
...
export default function FormularioUsuarioScreen({ navigation, route }: Props) {
  const usuarioExistente = route.params?.usuarioExistente as Usuario | undefined;
  const esEdicion = !!usuarioExistente;

  const [formData, setFormData] = useState({
    nombres: usuarioExistente?.nombres || '',
    apellidos: usuarioExistente?.apellidos || '',
    tipo_documento: usuarioExistente?.tipo_documento || 'CC',
  ...
  const cargarRoles = async () => {
    try {
      ...
      if (!esEdicion && listaRoles.length > 0 && formData.rol_id === 0) {
        setFormData(prev => ({ ...prev, rol_id: listaRoles[0].id }));
      }
    } catch (error) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      if (esEdicion) {
        ...
        await actualizarUsuario(usuarioExistente.id, updateData);
        mostrarAlerta('positivo', 'Usuario actualizado correctamente');
      } else {
        // Para creación, incluimos password (obligatorio) y omitimos empresa_id (backend lo asigna)
        const newUserData = {
          nombres: formData.nombres.trim(),
          apellidos: formData.apellidos.trim() || undefined,
          tipo_documento: formData.tipo_documento,
  ...
  const tiposDocumento = ['CC', 'NIT', 'CE', 'NUIP'];
    ...
    <View style={styles.container}>
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <TextInput style={styles.input} value={formData.nombres} onChangeText={text => setFormData({ ...formData, nombres: text })} placeholder="Nombres" editable={!loading} />
          ...
          <TextInput style={styles.input} value={formData.password} onChangeText={text => setFormData({ ...formData, password: text })} placeholder={esEdicion ? "Dejar vacío para no cambiar" : "********"} secureTextEntry editable={!loading} />
          ...
          <View style={styles.selectContainer}>
            {roles.map(rol => (
              <TouchableOpacity key={rol.id} style={[styles.selectOption, formData.rol_id === rol.id && styles.selectOptionActive]} onPress={() => setFormData({ ...formData, rol_id: rol.id })}>
                <Text style={formData.rol_id === rol.id ? styles.selectTextActive : styles.selectText}>{rol.nombre}</Text>
              </TouchableOpacity>
            ))}
          </View>
