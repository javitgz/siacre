...
export default function UsuariosScreen({ navigation }: Props) {
    ...
    const handleToggleActivo = async (usuario: Usuario) => {
        ...
        if (usuario.activo === nuevoEstado) {
            mostrarAlerta('negativo', `El usuario ya está ${usuario.activo ? 'activado' : 'desactivado'}`);
            return;
        }
        try {
                        ...
                        usuarios.map((usuario) => (
                            <View key={usuario.id} style={styles.usuarioCard}>
                                ...
                                <View style={styles.usuarioAcciones}>
                                    ...
                                    {/* Botón de activar/desactivar corregido */}
                                    <TouchableOpacity
                                        style={[
                                            styles.botonAccionIcono,
                                            usuario.activo ? styles.botonActivar : styles.botonDesactivar
                                        ]}
                                        onPress={() => handleToggleActivo(usuario)}
                                        activeOpacity={0.7}
                                    >
                                        <Ionicons
                                            name={usuario.activo ? 'lock-open' : 'lock-closed'}
                                            size={18}
                                            color={colors.blanco}
                                        />
                                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    botonActivar: { backgroundColor: colors.verdeEsmeralda },   // verde para activado
    botonDesactivar: { backgroundColor: '#dc3545' },           // rojo para desactivado
