...
// ─── INTERFAZ DEL LOG DE AUDITORÍA (SOPORTE SNAKE_CASE) ──────────────
...
// Tipado estricto para los nombres de íconos válidos de Ionicons
...
export default function AuditoriaScreen({ navigation }: Props) {
  ...
  const cargarLogs = async () => {
    try {
      const logsGuardados = await AsyncStorage.getItem('@auditoria_logs');
      if (logsGuardados) {
        setLogs(JSON.parse(logsGuardados) as LogAuditoria[]);
      } else {
        // Semilla de logs iniciales
        const logsEjemplo: LogAuditoria[] = [
          {
            id: '1',
            fecha: new Date().toISOString(),
            usuario: 'Javier Tamayo',
          ...
          },
          {
            ...
            modulo: 'Clientes',
            detalles: 'Cliente: Cliente Ejemplo',
          },
          {
            id: '3',
            ...
            modulo: 'Parámetros',
            detalles: 'Parámetro: Liquidez corriente',
          },
        ];
        setLogs(logsEjemplo);
        await AsyncStorage.setItem('@auditoria_logs', JSON.stringify(logsEjemplo));
      }
    } catch (error) {
  ...
  const logsFiltrados = logs.filter((log) => {
    ...
    return log.modulo.toLowerCase() === filtro.toLowerCase();
  });
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filtrosContainer}>
            {modulos.map((mod) => (
              <TouchableOpacity
                ...
                style={[
                  styles.filtroButton,
                  filtro === mod && styles.filtroButtonActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.filtroTexto,
                    filtro === mod && styles.filtroTextoActive,
                  ]}
                >
